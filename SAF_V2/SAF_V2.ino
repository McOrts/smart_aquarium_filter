...
void setup() {
  ...
  // Adjust only if your particular ServoFeeder does not reach the desired ends.
  ServoFeeder.setPeriodHertz(50);
  ServoFeeder.attach(PinFeeder, 500, 2500);
  // Start in a known position.
  ServoFeeder.write(0);
  delay(700);
  Serial.println("KC868-A6 ServoFeeder test started.");
...
void loop() {
  ...
  if (now - lastMsg > update_time) {
    ...
    // Water level sensor
    if (pcf8574_input.digitalRead(PinLevelSensor) == HIGH) {
